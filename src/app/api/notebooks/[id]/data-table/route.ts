...
import { generateDataTable } from '@/lib/ai/claude';
...
export async function GET(
  ...
  try {
    ...
    const result = db
      ...
      .where(
        and(
          ...
          eq(schema.studyAidResults.type, 'data-table')
        )
      )
    ...
    if (!result) {
      return Response.json({ tables: null });
    }

    return Response.json({
      id: result.id,
      tables: JSON.parse(result.result),
      createdAt: result.createdAt,
    });
  } catch (error) {
    console.error('Failed to get data tables:', error);
    return Response.json(
      { error: 'Failed to get data tables' },
      { status: 500 }
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    console.error('Failed to generate data tables:', error);
    return Response.json(
      { error: 'Failed to generate data tables' },
      { status: 500 }
    );
  }
}
