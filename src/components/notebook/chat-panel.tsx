...
function CitationBadge({
  ...
  return (
    <button
      ...
      className="inline-flex items-center justify-center min-w-[1.25rem] h-[1.125rem] px-1 mx-0.5 rounded-[10px] text-[10px] font-semibold bg-[var(--surface-container-high)] text-[var(--text-secondary)] hover:bg-[var(--surface-container)] border border-[var(--outline-variant)] transition-all duration-200 cursor-pointer align-text-top"
    >
      {index}
    </button>
  );
  ...
  return (
    <div className="flex justify-start">
      <div className="flex items-start gap-2.5">
        <div className="size-7 rounded-full bg-[var(--primary-container)] flex items-center justify-center shrink-0 mt-0.5">
          <Sparkles className="size-3 text-[var(--nlm-primary)]" />
        </div>
        <div className="bg-[var(--surface)] rounded-2xl rounded-tl-md px-4 py-3">
          <div className="flex items-center gap-1.5">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="size-1.5 rounded-full bg-[var(--text-secondary)]"
                style={{
...
function renderMarkdownInline(text: string): React.ReactNode {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[2]) {
      ...
      parts.push(
        <code key={match.index} className="px-1 py-0.5 rounded bg-[var(--surface-container)] text-[var(--nlm-primary)] text-[0.85em] font-mono">
          {match[6]}
        </code>
      );
    }
...
function renderMessageContent(
  ...
  lines.forEach((line, lineIdx) => {
      ...
      if (inCodeBlock) {
        elements.push(
          <div key={`code-${lineIdx}`} className="my-3 rounded-lg overflow-hidden">
            {codeBlockLang && (
              <div className="bg-[var(--surface-container)] px-3 py-1.5 text-[10px] text-[var(--text-secondary)] font-mono border-b border-[var(--outline-variant)]">
                {codeBlockLang}
              </div>
            )}
            <pre className="bg-[var(--surface-container)] px-3 py-3 overflow-x-auto">
              <code className="text-xs text-[var(--text-primary)] font-mono">{codeBlockContent.join('\n')}</code>
            </pre>
          </div>
        );
        codeBlockContent = [];
    ...
    if (trimmed.startsWith('### ')) {
      elements.push(
        <h4 key={lineIdx} className="text-sm font-semibold text-[var(--text-primary)] mt-3 mb-1.5">
    ...
    if (trimmed.startsWith('## ')) {
      elements.push(
        <h3 key={lineIdx} className="text-[15px] font-semibold text-[var(--text-primary)] mt-4 mb-1.5">
    ...
    if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
      elements.push(
        <div key={lineIdx} className="flex gap-2 ml-1 mb-0.5">
          <span className="text-[var(--text-secondary)] mt-0.5 shrink-0 text-[10px]">&#9679;</span>
    ...
    if (numberedMatch) {
      elements.push(
        <div key={lineIdx} className="flex gap-2 ml-1 mb-0.5">
          <span className="text-[var(--text-secondary)] text-xs shrink-0 w-4 text-right font-medium">
            {numberedMatch[1]}.
          </span>
          <span className="text-[13px] text-[var(--text-primary)] leading-relaxed">
    ...
    elements.push(
      <p key={lineIdx} className="text-[13px] text-[var(--text-primary)] leading-relaxed mb-1">
...
export function ChatPanel({
  ...
  return (
    <div className="flex-1 flex flex-col h-full min-w-0 bg-[var(--bg)]">
      {/* Messages */}
      <ScrollArea ref={scrollRef} className="flex-1">
        <div className="max-w-3xl mx-auto px-6 py-6">
          {isEmpty ? (
            <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fade-in">
              <div className="size-14 rounded-2xl bg-[var(--primary-container)] flex items-center justify-center mb-5">
                <Sparkles className="size-6 text-[var(--nlm-primary)]" />
              </div>
              <h2 className="text-xl font-medium text-[var(--text-primary)] mb-1.5 font-[family-name:var(--font-heading)] tracking-tight">
                Ask about your sources
              </h2>
              <p className="text-sm text-[var(--text-secondary)] mb-8 text-center max-w-md leading-relaxed">
                {hasSources
            ...
            <div className="space-y-5">
              {messages.map((msg) => (
                ...
                >
                  ...
                  {msg.role === 'user' && (
                    <div className="max-w-[75%] bg-[var(--primary-container)] text-[var(--text-primary)] rounded-2xl rounded-tr-md px-4 py-2.5">
      ...
      <div className="p-4 pb-5">
        <div className="max-w-3xl mx-auto">
            ...
            <div className="flex items-center gap-1.5 mb-2.5 px-1">
              {CHAT_STYLES.map((style) => (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-full text-[11px] font-medium transition-all duration-200 cursor-pointer border ${
                    (chatStyle ?? 'default') === style.value
                      ? 'bg-[var(--primary-container)] text-[var(--nlm-primary)] border-[var(--nlm-primary)]'
                      : 'bg-transparent text-[var(--text-secondary)] border-[var(--outline)] hover:bg-[var(--surface-container)]'
                  }`}
                >
                  {style.label}
                </button>
          ...
          <div className="flex items-end gap-2 bg-[var(--bg)] border border-[var(--outline)] rounded-3xl p-2.5 transition-all duration-200 focus-within:border-[var(--nlm-primary)] focus-within:shadow-[var(--shadow-1)]">
            <Textarea
              ref={textareaRef}
              value={input}
              ...
              className="flex-1 bg-transparent border-0 text-[var(--text-primary)] placeholder:text-[var(--text-secondary)] resize-none min-h-[36px] max-h-[120px] focus-visible:ring-0 focus-visible:ring-offset-0 p-1 text-sm"
              rows={1}
            />
            <Button
              ...
              size="icon"
              className="bg-[var(--nlm-primary)] hover:opacity-90 text-[var(--on-primary)] shrink-0 disabled:opacity-20 rounded-full size-8 transition-all duration-200"
            >
